...
本次研究最初覆盖 Donut Browser / HIL 浏览器钱包系统。后续，我又对 Donut 的 D0 / OpenClaw AI Agent 系统进行了独立安全研究。两阶段共整理出 **49 个安全问题**：HIL 系统 35 个，D0/OpenClaw 系统 14 个。其中包括跨用户交易构建、服务端自动签名链路、CORS + credentials 风险、钱包 IDOR、D0 控制面凭据泄露、OpenClaw 配置修改导致的 RCE、容器内 Session Token 与环境信息泄露等问题。
...
4. HIL 阶段跨用户资金攻击链在 Donut 后端/API 层成立，但实际第三方资金转出被 Turnkey AUTH001 等策略拦截。
...
| 2026 年 3 月 11 日 | 线上会议向 Donut 团队演示 HIL 资金安全攻击链，说明漏洞等级、攻击路径和披露流程 |
...
| 2026 年 3 月 16 日 | 复测确认 HIL 核心问题仍未修复，并继续补充漏洞清单 |
...
Donut Browser / HIL 系统的核心风险集中在“后端替用户构建并执行交易”这条链路上。该系统试图让 AI 或后端服务代替用户完成交易构建与执行，但后端没有建立足够严格的用户隔离、钱包归属校验和二次授权边界。
...
| 编号 | 漏洞 | 严重性 |
|---|---|---|
| HIL-1 | 服务端交易自动签名 / 执行链路缺失强授权边界 | Critical |
| HIL-2 | 跨用户交易构建，交易参数未绑定当前会话钱包归属 | Critical |
| HIL-3 | 任意钱包 / 资产相关接口 IDOR | Critical |
| HIL-4 | CORS 子域策略与 credentials 组合导致跨域攻击面扩大 | Critical |
| HIL-5 | MCP 相关接口认证与权限边界不足 | Critical |
| HIL-6 | Role 参数注入导致 AI / Agent 行为边界绕过 | High |
| HIL-7 | 无认证或弱认证钱包创建 | High |
| HIL-8 | Credits / 使用次数限制绕过 | High |
| HIL-9 | AI Agent 配置和 Prompt 泄露 | High |
| HIL-10 | 跨用户限价单取消 | High |
| HIL-11 至 HIL-35 | 信息泄露、缺失速率限制、安全头缺失、错误信息暴露等问题 | Medium / Low |
...
D0/OpenClaw 阶段是我在 2026 年 5 月后续独立完成的安全研究。它与 3 月 HIL 阶段不同，不是 Donut 在 3 月会议后持续配合我开展的修复验证工作。

我选择将其纳入 2026 年 6 月 9 日披露，是因为：

1. D0 属于 Donut 同一产品生态下的核心 AI Agent 系统。
2. HIL 阶段沟通已经表明 Donut 对系统级安全问题的处理不符合我对专项安全事件的最低预期。
...
2026 年 3 月 21 日，我基于 HIL 阶段问题提交了 9 个 CVE 申请。正式发布版本会列出 ticket ID、漏洞映射、CWE 和最终统一后的 CVSS 信息。
...
1. HIL 阶段测试发生在 Donut 官方 Bug Hunt 活动和后续线上沟通框架内。
