...
- **Who**: independent security researcher Lucifiel, a Donut user and beta tester since 2025-12.
...
> The advice below is based on objective facts: as of the June 2026 retest, of the 49 issues compiled here only 11 are fully fixed, 9 partially fixed, and 29 still unfixed (including several core Criticals), and as of publication no third-party security audit or remediation-retest report from Donut has been seen. The items below are general crypto-asset security-hygiene measures that apply to any early-stage product that connects wallets and automated execution — they are not aimed at any specific party; whether to adopt them is your own judgment.
...
Donut is an AI + Crypto product ecosystem whose product directions include a browser, a wallet, on-chain trading assistance, an AI Agent, and automated execution. Public information indicates Donut Labs completed two funding rounds within roughly six months of founding, totaling 22 million USD (Pre-seed ~7 million + Seed 15 million), with investors and ecosystem backing including BITKRAFT, Makers Fund, HSG / HongShan, Sky9 Capital, MPCi, Altos Ventures, Hack VC, as well as Solana, Sui, Monad, Jupiter, Drift, Manifold Trading, and others. Its core narrative is to let users access on-chain assets, strategy execution, and trading in a more natural way.

Donut Browser serves as the browser and wallet entry point, touching user wallets, asset queries, transaction building, DeFi operations, limit orders, and the backend transaction-execution path. D0 is the AI-Agent platform Donut offers externally, where users can configure and run Agents in an isolated environment. During the research, after obtaining control-plane access material through a D0 vulnerability, the researcher accessed the control plane pointed to by the corresponding IP, port, and Token; the page clearly displayed the OpenClaw logo, and the docs link also pointed to the OpenClaw official documentation. This is therefore not an inference based on behavioral similarity — it can be confirmed that D0 is built on OpenClaw underneath, with secondary development or product packaging on top. When promoting D0 externally, Donut did not present OpenClaw as primary product information, which makes D0's degree of in-house development, the transparency of its open-source dependency, and the security review of its secondary development matters worth attention.

The security risks of this kind of product differ from those of an ordinary web application. An ordinary web app's authorization flaw may lead to data leakage or account-feature abuse; but once an AI + Crypto product connects wallets, signing services, automated trading, and a container runtime, an authorization flaw can directly affect asset safety, session safety, and the user's control over their runtime environment.

So for a system like Donut, the security boundary should at minimum include: binding backend user identity to wallet ownership, strong authorization confirmation before transaction execution, least-privilege control, sensitive-credential isolation, Agent runtime-environment isolation, control-plane capability auditing, and an anomalous-operation interception mechanism.

This disclosure discusses precisely the systemic problems these boundaries exposed in actual testing.
...
Each piece of evidence is clickable: [Three meeting records](./evidence/meetings.md) · [Full Telegram record](./evidence/telegram.md) · [Solana on-chain transaction evidence](./evidence/chain.md) · [Donut Browser evidence](./evidence/donut-browser.md) · [D0 evidence](./evidence/d0.md).
...
Related evidence: [Donut Browser vulnerability evidence](./evidence/donut-browser.md).
...
Related evidence: [D0 AI-Agent automated-trading-system vulnerability evidence](./evidence/d0.md).
...
**Q1: As a product that connects wallets, signing, and real-time online automated-trading capabilities, with "security first" emphasized in its website and marketing, why does Donut still expose basic security problems such as user authorization, wallet-ownership validation, control-plane isolation, and runtime-environment isolation?**

**Q2: Donut Browser has a verified 1-click silent transaction chain that can — with no authorization or signature confirmation by the user — bypass the signing-authorization step and transfer all funds from the wallet. Does this actual performance meet the security standard an automated-trading product claims publicly?**

**Q3: D0 has a verified path from an ordinary login state to obtaining control-plane connection material, further combined into config read, config modify, file write, own-Pod RCE, and reading sensitive session material. Does this control-plane exposure meet the security boundary an AI-Agent product should have?**

**Q4: After the researcher submitted major security research, took part in meetings, provided a redacted report, slides, and a PoC video, and clearly gave a responsible-disclosure window, the vendor still downgraded Critical-level vulnerabilities to ordinary-community-event handling. Did that give the security issues themselves, and the security researcher, sufficient respect?**

**Q5: If an independent security researcher submits major vulnerability findings under a responsible-disclosure framework and is ultimately downgraded to ordinary-community-event handling, will this make more researchers lose confidence in compliant disclosure?**

**Q6: When a researcher's security findings for a project are not respected, and they become disappointed about continuing compliant research, real attackers and underground actors will not care whether they are respected. Does this attitude instead give attackers a bigger opening?**

**Q7: For a project with such high funding and market expectations, its core AI-Agent product D0 is built by secondary development on an open-source product that has been live for only about half a year. Is that worthy of its funding scale and market expectations? Does the potential security risk it brings match the "security first" Donut keeps stating?**
...
Fifth, open-source dependency and secondary development are not themselves a problem. The problem is that a company disclosing 22 million USD in seed-round funding and presenting D0 as its core AI-Agent product has a core control plane that, on actual access, clearly displays the OpenClaw logo, with the docs link pointing to the OpenClaw official documentation. This fact should be discussed openly: there is a clear gap between D0's external packaging, its in-house-development narrative, the transparency of its key open-source dependency, the security review of its secondary development, and its actual security-engineering capability. What truly deserves scrutiny is not the use of open source, but that, under a high-funding narrative, the core product's dependency on OpenClaw was not adequately disclosed, nor was a security-engineering and vulnerability-response capability commensurate with the funding scale demonstrated.
...
Lucifiel (Chinese ID 路西菲尔). According to the public blog's About page, Lucifiel has long kept a habit of technical record-keeping and maintains an interest in technology, the unknown, and offensive-defensive research.

My focus is on Web3 security, AI-Agent security, business-logic vulnerabilities, offensive-defensive research, and the privilege-boundary and trust-model problems of complex systems. This Donut AI vulnerability disclosure is part of my systematic research into the security boundaries of AI + Crypto products.

I currently welcome contact in the following directions:

1. Suitable job opportunities, especially roles in security research, business-logic security, Web3 security, AI-Agent security, and offensive-defensive research.
2. Security-service cooperation, including business-logic vulnerability hunting, Web3 / AI-Agent security assessment, attack-chain analysis, risk reports, and security advisory.
3. Media interviews, technical reporting, community talks, and industry-observation cooperation.
4. Product, service, and research-oriented project cooperation around business-logic security.
5. Investor conversations interested in business-logic security, AI-Agent security, or Web3-security product directions.
